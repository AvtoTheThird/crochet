...
export function drawColorPreview() {
  ...
  dom.mainCtx.imageSmoothingEnabled = false;
  dom.mainCtx.clearRect(0, 0, dw, dh);
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      dom.mainCtx.fillStyle = cell.hex;
      dom.mainCtx.fillRect(x0, y0, Math.max(1, x1 - x0), Math.max(1, y1 - y0));
    }
  }
}
