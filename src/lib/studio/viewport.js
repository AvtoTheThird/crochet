...
function applyCssDisplaySize(dw, dh, anchor) {
	...
	if (anchor) {
		...
		dom.canvasArea.scrollLeft = Math.max(0, contentX * ratioX - (anchor.x - rect.left));
		dom.canvasArea.scrollTop = Math.max(0, contentY * ratioY - (anchor.y - rect.top));
	}
...
export function setGridZoom(value, anchorX, anchorY) {
	...
	state.zoomLevel = Math.max(0.25, Math.min(8, Number(value) / 100 || 1));
