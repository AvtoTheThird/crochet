...
export function updatePatternWalkUI() {
  ...
  progressEl.textContent = `Stitch ${idx + 1} of ${total}`;
  if (detailEl) {
    detailEl.innerHTML = `<span class="walk-color-swatch" style="background:${step.segment.color.hex}"></span> <strong>${label}</strong> × ${step.segment.count}`;
  }
  if (rowEl) {
